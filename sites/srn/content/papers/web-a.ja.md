...
1.  **Layer 1: スキーマ (The Schema)**
    ...
    *   **内容**: JSON-LDコンテキスト、バリデーションロジック、文書構造定義。上位互換性を維持する限り安定的に運用される。
...
4.  **Layer 4: プレゼンテーション (The Presentation / Container)**
    *   **役割**: 視覚的表現と現在の真正性。
    *   **内容**: **HTMLテンプレート**、CSS、フォント、およびL3までの全データを含むコンテナ。
    *   **特性**: **コンテナ署名 (Ephemeral)**。リビルドごとに再生成される署名であり、最新の「見え方（テンプレート実装）」の正当性を証明するが、核となるLayer 1/2構造には影響を与えない。
