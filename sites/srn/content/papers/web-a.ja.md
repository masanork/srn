...
4.  **Layer 4: プレゼンテーション (The Presentation / Container)**
    *   **役割**: 視覚的表現と現在の真正性。
    *   **内容**: HTML構造、CSS、フォント、およびL3までの全データを含むコンテナ。
    *   **特性**: **コンテナ署名 (Ephemeral)**。デプロイごとに再生成される署名であり、最新の「見え方」の正当性を証明するが、核となるLayer 2データには影響を与えない。
...
**Web/A は「階層化署名」でこれを解決する：**
*   **Payload Signature (L2)**: 作成時に一度だけ署名される。この署名ブロックはバイト単位で新しい HTML テンプレートへ移植される。
*   **Container Signature (L4)**: リビルドのたびに生成される。新しい HTML 構造を保証するが、L2 の署名時刻には影響を与えない。
これにより、「2025年に発行された事実」を維持したまま、「2030年の最新デザインで表示する」ことが可能になる。
