...
- **2026-01-01**: 長期検証 (LTV) アーキテクチャを定義。「階層化署名」によるリビルド耐性の確立と、オフライン検証モデルの追加 (v2.0)
- **2026-01-02**: マニフェスト・アーキテクチャ (Pack & Prune) を導入。Layer 1 を Core と Manifest に分離し、リソース管理の効率化と検証性の両立を図る (v2.1)
...
フォント、画像、スタイルは Data URL 等によりインライン化、または同梱される。数十年後のブラウザでも、外部サーバーにアクセスすることなく機能することが保証される。
...
後述する「4層トラスト・アーキテクチャ」により、発行者が提供する「正しい設問とロジック（Layer 1）」に対し、利用者が「署名付きの回答（Layer 2）」を付加することで、双方の真正性が担保された一連の文書レコードが完成する。
...
1.  **Layer 1: スキーマ (The Schema)**
    *   **役割**: 文書の型定義、設問構造、および意味論の規定。
    *   **構造**: **L1-Core**（不変のロジック・スキーマ）と **L1-Manifest**（可変・参照可能なリソース）に分離される。
    *   **形式**: **人間が読める Markdown 仕様書** を正本 (Source of Truth) とする。
    *   **内容**: Markdownから導出されたJSON-LDコンテキスト、JSON Schema、バリデーションロジック、および外部Blob（フォント、辞書）への参照。
    *   **署名者**: **発行者 (Issuer)**。組織による「定義された事実の型」を保証する。
    *   *注記*: ファイルサイズと更新頻度の課題に対処するため、重いアセットは **[マニフェスト・アーキテクチャ](./web-a-manifest-architecture.ja.html)** (Pack & Prune 戦略) によって管理される。

#### 6.1.1. L1 (Schema) の記述と導出
Web/A では、HMP (Human-Machine Parity) を徹底するため、スキーマの定義ファイル自体が人間によって可読・編集可能（Human-writable）でなければならない。

*   **Markdown Schema**: 表形式などでデータ構造を定義する。これが L1 ハッシュの根拠となる。
*   **Derivation (導出)**: ビルドプロセスにおいて、Markdown から JSON Schema や JSON-LD Context を自動生成（コンパイル）する。
*   **Pluggable Validation**: 生成されたアーティファクトを用いて、AJV 等のバリデータが Layer 2 データの整合性をチェックする。スキーマ言語自体を Pluggable に保つことで、将来の標準規格の変更に追従する。
...
3.  **Layer 3: コンテキスト (The Context / Chain of Custody)**
    *   **役割**: 監査証跡と管理履歴の統合。
    *   **内容**: **Prunable Hash Chain (PHC)**。タイムスタンプ、配送タグ、および失効情報（CRL/OCSP）の履歴。
    *   **特性**: 署名の連鎖（リンクリスト）により、「誰がいつ保持したか」の完全な履歴を証明する。中間サーバーが履歴を静かに削除することを防ぐ。

4.  **Layer 4: プレゼンテーション (The Presentation)**
    *   **役割**: 視覚的表現と現在の真正性。
    *   **実体 (Entity)**: L1〜L3の全データと、HTMLテンプレート、CSS、フォントを包含する**コンテナ**。
    *   **署名**: **コンテナ署名 (Container Signature)**。リビルドごとに再生成される署名であり、最新の「見え方（テンプレート実装）」と「コンテナ全体」の整合性を証明する。核となるLayer 1/2構造には影響を与えない。
...
### 6.4. 長期検証 (LTV) アーキテクチャ
発行元の署名は「誰が」発行したかを証明するが、標準的な Web 署名は「リンク切れ」や「コンテキストの消失」によって数年で検証不能になりがちである。Web/A は以下の LTV 戦略を採用する：

#### 6.4.1. "Rebuild Paradox" の解決
Web サイトの運用では、共通フッターの更新やブラウザ互換性のための修正などで HTML 全体をリビルドすることが頻繁にある。従来の電子署名では、HTML が 1bit でも変われば全ての署名が無効化されてしまう。
**Web/A は「階層化署名」と「L3追記型構造」でこれを解決する：**

Web/A では 4つの層すべてに署名またはハッシュによる検証機構が存在するが、その更新サイクルは異なる。

*   **L1 (Schema) & L2 (Payload)**: **不変 (Immutable)**。発行時に固定され、バイト単位で維持される（スキーマのバージョンアップは上位互換で行われる）。
*   **L3 (Context)**: **追記型 (Append-Only)**。Prunable Hash Chain により、リビルドや配布の履歴をブロックチェーンのように追記していく。「プレゼンテーション（L4）の更新」も一つのイベントとして記録される。
*   **L4 (Presentation)**: **更新可能 (Mutable)**。リビルドのたびに、その時点での L1+L2+L3 を、最新の HTML/CSS 実装 で包む形で新しいコンテナ署名が生成される。

これにより、「2025年に発行された事実（L2）」と「発行から現在までの履歴（L3）」を維持したまま、「2030年の最新デザインで表示する（L4）」ことが可能になる。

#### 6.4.2. オフライン検証（Trust Store Embedding）
数十年後、発行元の Web サイト (DID) は消滅しているかもしれない。
*   **埋め込みトラストストア**: 発行者の DID ドキュメントや、将来の失効情報 (CRL) を HTML ファイル内の `<script>` タグとして直接注入する。
*   **自己完結した信頼**: 外部サーバーへ問い合わせることなく、ファイル単体でオフライン検証が可能となる。

#### 6.4.3. Context Freezing
Web/A は **JCS (JSON Canonicalization Scheme, RFC 8785)** を採用し、検証時に外部の `@context` ファイルをフェッチする必要を排除した。これにより「スキーマ定義ファイルの 404 エラー」による検証失敗（Context Rot）を防ぐ。
...
<svg width="600" height="420" viewBox="0 0 600 420" fill="none" xmlns="http://www.w3.org/2000/svg" style="max-width: 100%; height: auto; border: 1px solid #e2e8f0; border-radius: 8px; background: #fff;">
  ...
  <text x="75" y="202" font-family="system-ui" font-size="9" fill="#065F46">暗号化ペイロード、利用者署名</text>
...
> [!TIP]
> **詳細資料**: L2Eの具体的な仕組みについては [Web/A L2 Encryption](./web-a-l2-encryption.ja.html) を、そのセキュリティ評価については [Security Audit v2 (English)](../governance/web-a-l2-security-audit-v2.html) および [既存製品との比較分析](./web-a-l2-market-comparison.ja.html) を参照してください。
...
数十年以上の長期保存において、署名を最新の暗号アルゴリズムへ洗替（Refresh）するプロセスの標準化。
...
- **課題**: 「数十年後のブラウザでも確実に動作する」と確信できる HTML/CSS の機能範囲（サブセット）を特定し、コミュニティでのコンセンサスを得ること。
