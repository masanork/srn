...
## 本人性：Passkey × National ID 署名

汎用的なWeb環境で安全な鍵管理を行う手段として **Passkey (WebAuthn)** を活用します。さらに、国家が担保する署名基盤（マイナンバーカード等）を用いて Passkey の公開鍵に署名を行うことで、日常的な利便性と厳密な本人証明を両立させます。

1.  デバイスで Passkey を生成。
2.  National ID でその公開鍵を証明。
3.  日常的な操作は生体認証（Touch ID/Face ID）だけで完結しつつ、その署名は National ID と同等の信頼性を間接的に証明可能となります。
