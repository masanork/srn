...
### 2.4. デザインと振る舞いのカスタマイズ (Look & Feel)

Web/A Form は、ビルド時に標準で「Premium by Default」なデザインシステム（CSS）を自動的に埋め込みます。これにより、単なるMarkdownの表組みであっても、行政文書や金融帳票として通用する品質の見た目が保証されます。

一方で、各フォーム固有の事情に合わせて、Frontmatter（ヘッダ部分）に記述することで、デザインや挙動をファイル単位で上書き・拡張できます。

- **`style`**: カスタムCSSを注入し、テーマカラーの変更、特定要素の非表示、余白の微調整などを行います。
- **`script`**: カスタムJavaScriptを注入し、独自のバリデーションロジックの追加や、標準機能のパッチ（修正）を行います。

この機能は、Web/A の**「自己完結性 (Self-contained)」**と**「自己修復性 (Self-repairing)」**を強化します。
たとえシステム側のランタイムに不具合があったり、特定の機能が欠けていたりしても、フォーム作成者がそのファイル内で「パッチ」を当てることで、システムの更新を待たずに問題を解決し、業務を継続できます。
