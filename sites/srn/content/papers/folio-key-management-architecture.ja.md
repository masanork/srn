...
| **LoA (Assurance)** | 高（High）レベルの保証。SE への依存が必須。 | **Holder Binding** により、Passkey と公的身分証を紐付けて保証。 |
...
*   **保管場所**: マイナンバーカード等の公的身分証（物理カード）+ Passkey。
*   **用途**: 法的効力を持つ手続き、高額取引。
*   **鍵管理**: 公的身分証の署名用電子証明書を用いて、Tier 2 の Passkey に対して「これは私の所持している鍵である」という署名を付与（**Holder Binding**）。
*   **メリット**: カードを毎回読み取る手間を省きつつ、法的・物理的な本人性を担保する。
