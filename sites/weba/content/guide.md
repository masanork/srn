...
*   **attributes**: スペース区切りで記述。
    ...
    *   `copy:key`: 指定したキーの値が変更されると、自動的にこのフィールドにコピーされます（編集済みでない場合のみ）。
...
*   `number` および `calc` タイプは、デフォルトで右寄せ (`align:R`) されます。
...
他のフィールドの値や計算結果を、初期値として自動的に反映させたい場合に使用します。
ユーザーが手動で値を変更した場合、自動コピーリンクは解除され、ユーザーの入力値が優先されます。

```markdown
- [number:prev_year] 前年度
- [number:this_year (copy:prev_year)] 今年度（初期値は前年度と同じ）
```

*   **視覚的フィードバック**: 自動コピーが有効なフィールド（未編集の状態）は、背景色が薄い黄色になり、自動入力された値であることが分かります。
...
*   **suggest**: `suggest:column` (属性) を指定すると、マスタデータだけでなく、テーブル内の他の行に入力された同列の値も候補として表示します（ハイブリッド検索）。
...
*   **自動コピー(Auto-Copy)**: `copy` 属性を持つフィールドに対し、参照元の値が変更されると即座に値を同期します。計算結果 (`calc`) からのコピーもサポートされます。
