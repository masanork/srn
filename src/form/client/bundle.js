...
function initAggregatorBrowser() {
  ...
  const parquetProvider = globalThis.webaParquet;
...
// src/form/client/parquet_provider.ts
var PARQUET_INIT_FLAG = "__webaParquetInit";
var isParquetReady = () => {
  return !!globalThis.parquetWasm;
};
var ensureInit = async (wasm) => {
  if (typeof wasm.init !== "function")
    return;
  if (wasm[PARQUET_INIT_FLAG])
    return;
  await wasm.init();
  wasm[PARQUET_INIT_FLAG] = true;
};
var exportParquet = async (records) => {
  const wasm = globalThis.parquetWasm;
  if (!wasm) {
...
function registerParquetProvider() {
  if (globalThis.webaParquet)
    return;
  const provider = {
    export: exportParquet,
    isReady: isParquetReady
  };
  globalThis.webaParquet = provider;
}
