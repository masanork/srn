        ...
        <article class="weba-article${presentationEnabled ? ' presentation-enabled' : ''}">
            ...
            <header>
                ...
                <div class="article-meta">
                    ...
                    ${data.updated ? `<span class="updated-date" title="Last modified date"><span data-i18n-ja="最終更新: " data-i18n-en="Updated: ">Updated: </span><time datetime="${data.updated}">${data.updated}</time></span>` : ''}
                    ...
                    ${Array.isArray(data.schemas) && data.schemas.length > 0 ? `
                        <span class="schema-compliance" title="Document Schema Compliance">
                            📜 <span data-i18n-ja="準拠スキーマ: " data-i18n-en="Compliant with: ">Compliant with: </span>
                            ${data.schemas.map((s: string) => `<code style="font-size: 0.8em; opacity: 0.8;">${s.split('/').pop()}</code>`).join(', ')}
                        </span>
                    ` : ''}
                </div>
            </header>
